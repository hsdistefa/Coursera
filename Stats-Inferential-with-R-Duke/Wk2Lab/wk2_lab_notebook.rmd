...
```{r set-seed}
set.seed(8733)                 # make sure to change the seed
```
...
**The distribution is right skewed, with the typical (median) size around 1400ft^2^**
```{r describe-sample}
...
**C, the sample distribution does not have to be nearly normal, only the sample mean**
...
lower <= mean(ames$area) && mean(ames$area) <= upper
```
...
**46/50=.92 of the confidence intervals include the true population mean. This is not exactly equal to the confidence level because we expect some variance, but the proportion should converge to 95% as the number of tests approaches infinity**
...
**E, qnorm(.995) = 2.5758 We use .995 because each of the two tails represent half of the non-confident area under the curve.**
...
ci <- ames %>%
        rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
        summarise(ci_99_lower = mean(area) - z_star_99 * (sd(area) / sqrt(n)),
                  ci_99_upper = mean(area) + z_star_99 * (sd(area) / sqrt(n)))

ci_data$ci_bounds <- c(ci$ci_99_lower, ci$ci_99_upper)
ci_data$capture_mu <- ifelse(ci$ci_99_lower < params$mu & ci$ci_99_upper > params$mu, "yes", "no")
