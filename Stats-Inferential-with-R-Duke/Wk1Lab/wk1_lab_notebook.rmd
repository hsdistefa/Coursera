...
**B is false, 50% of houses in Ames are smaller than 1442 ft^2^, the median**
...
**As you can see from the histogram and statistic summaries, the sample distribution is slightly right skewed similar to the population distribution, but with less variability as you can see with more narrow IQR and smaller standard deviation.**
...
**C, a sample size of 1000 should provide a more accurate estimate of the population mean since the standard error decreases as sample size increases**
...
**There are 15,000 elements in 'sample_means50', where each element is the mean of one of the 15,000 samples. The distribution is nearly normal with mean around the center as can be seen in the histogram below**
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
...
**There are 25 observations in 'sample_means_small' where each observation represents the sample mean area of a sample of size 10 from the Ames data set**
...
**As the sample size increases, the mean moves non-monotonically closer to the population mean, while the standard error decreases and the sampling distribution gets narrower. Increasing the number of simulations has a similar effect on the mean, while the standard error and shape converge to the normal distribution.**
```{r shiny, echo=FALSE}
shinyApp(
  ui <- fluidPage(
...
**A, as the sample size increases, the variability of the sampling distribution decreases as each sample mean will be more likely to fall close to the population mean**
...
**The shape is normal, unimodal, and the mean looks to be around $180,000**
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50 <- ames %>%
...
**This distribution looks very similar to the one with sample size of 50 albeit with less variability, the mean is still around $180,000**
...
# type your code for this Exercise here, and Run Document
price_sample_15 <- ames %>%
                    sample_n(size = 15) %>%
                    summarise(x_bar = mean(price))

print(price_sample_15)
```
...
**The distribution looks slightly right skewed and the mean looks to be around $178,000**
...
**C is false, the distributions are slightly right-skewed, especially the one with smaller sample size**
